...
export const consolidateHTMLConverters = ({
  ...
  const htmlConverterFeature =
    editorConfig.resolvedFeatureMap.get('htmlConverter')
...
export const lexicalText: (
  ...
  const { name = 'lexicalText', hidden = true, storeInDB = false } = props
  return {
    name,
    type: 'textarea',
    admin: {
    ...
    hooks: {
      afterRead: [
        async ({ collection, field, global, siblingData }) => {
          ...
          if (!fields)
            throw new Error('Could not find fields from config or globals')

          const foundSiblingFields = findFieldPathAndSiblingFields(
            fields,
            [],
            field,
          )
          ...
          const lexicalField: RichTextField<
          ...
          const lexicalFieldData: SerializedEditorState =
            siblingData[lexicalFieldName]
          ...
          if (!lexicalField) {
            throw new Error(
              'You cannot use the lexicalText field because the referenced lexical field was not found',
            )
          }

          const config = (lexicalField?.editor as LexicalRichTextAdapter)
            ?.editorConfig

          if (!config) {
            throw new Error(
              'The linked lexical field does not have an editorConfig. This is needed for the lexicalText field.',
            )
          }

          if (!config?.resolvedFeatureMap?.has('htmlConverter')) {
            throw new Error(
              'You cannot use the lexicalText field because the linked lexical field does not have a HTMLConverterFeature',
            )
          }
            ...
            headlessEditor.update(
              () => {
                headlessEditor.setEditorState(
                  headlessEditor.parseEditorState(lexicalFieldData),
                )
              },
